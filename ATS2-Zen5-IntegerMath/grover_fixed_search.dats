...
parallel_diffusion_zen5 (
  ...
  fun
  reflect_and_scan {i:nat | i <= N} .<N-i>. (
...
fun
find_peak_safe (amplitudes: &array(Fixed_Amplitude, N),
                peak_index: &Index? >> Index)
    : void =
  let
    fun
    scan {i:nat | i <= N} .<N-i>.
...
main0 () =
  let
    ...
    fun
    populate {i:nat | i <= N} .<N-i>.
             (pf: !array_v (int, data, N) |
              i: size_t (i))
        : void =
      if i < N then
        let
          // Background array value noise
          val () = array_set_at_guarded (pf | data, i, 123)
        in
          populate (pf | i + 1)
        end
      else
    ...
    fun
    init_amp {i:nat | i <= N} .<N-i>.
             (pf: !array_v(int, amplitudes, N) |
              i: size_t i)
        : void =
      if i < N then
        let
    ...
    fun
    run_iterations (step: int)
        : void =
      if step <= CEILING_ITERATIONS then
        let
          val () = actual_steps := step
          val () = parallel_oracle_zen5
                     (pf_data | data, 4242, !amplitudes)
          val () = parallel_diffusion_zen5
                     (!amplitudes, match_found, target_index)
        in
          if not match_found then
            run_iterations (step + 1)
          else
            ()
        end
      else
    ...
    val () =
      if not match_found then
        find_peak_safe (!amplitudes, target_index)
    val confirmed_data =
      array_get_at_guarded (pf_data | data, target_index)
  in
    if confirmed_data = 4242 then
      begin
        ...
        println! ("Hardware clock cycles consumed: ",
