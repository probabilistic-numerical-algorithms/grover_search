...
blindly for \(R \approx \frac{\pi}{4}\sqrt{N}\) steps until a destructive
measurement collapses the vector.

This circuit strips away that vocabulary. We represent a 2-bit database
(\(N = 2^{2} = 4\) states) as four physical, concurrent AC voltage
signals oscillating at a fixed carrier frequency (\(f_0
\approx 16\text{ kHz}\)).
...
\[
V_{\text{out\_channel}} = 2V_{\text{mean}} -
V_{\text{in\_channel}}
\]
