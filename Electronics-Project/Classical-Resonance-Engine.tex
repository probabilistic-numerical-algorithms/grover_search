...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{* Classical Grover Resonance Engine {-} Stabilized Rail{-}Clipped Netlist}
\NormalTok{.OPTIONS plotwinsize=0 numdgt=7 nopage}
\NormalTok{.PARAM FREQ=15915 RMATCH=10k}

\NormalTok{* 1. NATIVE IDEAL DIODE MODEL }
\NormalTok{.MODEL D\_IDEAL D(Is=1e{-}14 N=1)}

\NormalTok{* 2. Force persistent data retention for plotting}
\NormalTok{.SAVE V(Ref\_CLK) V(Node\_2\_Oracle) V(Mean\_Out) V(Envelope\_0)}

\NormalTok{* 3. Carrier Signal Reference Generator (1V Peak Reference)}
\NormalTok{V\_Ref Ref\_CLK 0 SINE(0 1 \{FREQ\})}
\NormalTok{R\_Ref\_Load Ref\_CLK 0 100k}

\NormalTok{* 4. State Channel Array Buffers (Unity Gain Reference Buffers)}
\NormalTok{E\_Buf0 Node\_0\_In 0 Ref\_CLK 0 1}
\NormalTok{E\_Buf1 Node\_1\_In 0 Ref\_CLK 0 1}
\NormalTok{E\_Buf2 Node\_2\_In 0 Ref\_CLK 0 1}
\NormalTok{E\_Buf3 Node\_3\_In 0 Ref\_CLK 0 1}

\NormalTok{* 5. Mechanical Oracle Matrix (State 2 Phase Flipped {-} Clipped at +/{-}12V Rails)}
\NormalTok{E\_Inv2 Node\_2\_Op\_Out 0 VALUE=\{LIMIT(100000 * (0 {-} V(Node\_2\_In)), {-}12, 12)\}}
\NormalTok{R\_Inv\_In   Node\_2\_In     Node\_2\_Oracle \{RMATCH\}}
\NormalTok{R\_Inv\_Feed Node\_2\_Op\_Out Node\_2\_Oracle \{RMATCH\}}

\NormalTok{* 6. Central Diffusion Summing Mixer (Computes Mean {-} Clipped at +/{-}12V Rails)}
...
\NormalTok{E\_Mixer Mean\_Out 0 VALUE=\{LIMIT(100000 * (0 {-} V(Sum\_Node)), {-}12, 12)\}}
\NormalTok{R\_Feedback Sum\_Node Mean\_Out \{RMATCH\}}

\NormalTok{* 7. Envelope Demodulator}
\NormalTok{D\_Det0 Node\_2\_Oracle Envelope\_0 D\_IDEAL}
\NormalTok{C\_Smooth0 Envelope\_0 0 10nF}
\NormalTok{R\_Bleed0 Envelope\_0 0 100k}

\NormalTok{.TRAN 0 10m 0 1u}
\NormalTok{.END}
\end{Highlighting}
\end{Shaded}
